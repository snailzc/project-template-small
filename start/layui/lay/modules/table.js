      ...
      ,text: {
        none: '暂无数据'
          ...
          if(res.data == null || res.data.length == 0){
            // debugger;
            $('div[lay-id="' + this.id + '"] .layui-table-header') && $('div[lay-id="' + this.id + '"] .layui-table-header').css("overflow-x","scroll");
          }
      }
    };
      ...
      if(options.url){ //Ajax请求
        ...
        $.ajax({
          ...
          ,error: function(e, m){
            ...
            if(e.responseJSON['status'] === 4010001){
                layer.msg('登录状态已失效', {
                    offset: '15px'
                    ,icon: 2
                    ,time: 1500
            ...
            else if(e.responseJSON['status'] === 4000003){
              layer.msg('您暂无权限', {
                  offset: '15px'
                  ,icon: 2
                  ,time: 1500
                },function(){
                  that.errorView('您暂无权限：'+ e.responseJSON.message);
                  that.renderForm();
                  that.setColsWidth();
                });
            }
            else{
                that.errorView('数据接口请求异常：'+ e.responseJSON.message);
                that.renderForm();
                that.setColsWidth();
            }            
          }
