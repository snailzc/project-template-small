...
layui.define('form', function (exports) {
  ...
  Class.prototype.spread = function (elem, item) {
    ...
    elemText.on('click', function () {
      var othis = $(this);
      let layuiTree = othis.closest(".layui-tree");
      layuiTree.find(`.${ELEM_TEXT}`).removeClass(`${ELEM_TEXT}-active`);
      othis.addClass(`${ELEM_TEXT}-active`);
      //判断是否禁用状态
  ...
  Class.prototype.setCheckbox = function (elem, item, elemCheckbox) {
    ...
    if (options.parentUpdateChildren) {
      //同步子节点选中状态 Hao ZhiQiang 注释于2019/9/25日，目的：组织点击父节点时选中子节点
  ...
  Class.prototype.getChecked = function () {
    ...
    //遍历节点找到选中索引
    let checkNodes = that.elem.find('.layui-form-checked');
    checkNodes.each(function () {
      checkIds.push($(this).prev()[0].value);
    });

    function unfolddArr(arr) {
      let dataArr = [], l = arr.length;
      for (let i = 0; i < l; i++) {
        let item = arr[i];
        dataArr.push(item);
        if (item.children && item.children.length > 0) {
          dataArr = dataArr.concat(unfolddArr(item.children))
        }
      }
      return dataArr;
    }

    let checkData = unfolddArr(options.data).filter(item => checkIds.includes(item.id));
    return checkData;
  };
