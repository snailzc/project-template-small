...
gulp.task("rebuild", function(callback) {runSequence('clear', 'minjs', 'mincss', 'mv', 'reload')(callback)});
...
//构建核心源文件
gulp.task('default', ['clear', 'src', 'server'], function(){ //命令：gulp
    for(var key in task){
        task[key]();
    }
});

gulp.task('pro', ['clear', 'src'], function(){ //命令：gulp
    for(var key in task){
        task[key]();
    }
});
