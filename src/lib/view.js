...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.req = function(options){
    ...
    if(request.tokenName){
      ...
      options.headers['Authorization'] = request.tokenName in options.headers 
    ...
    return $.ajax($.extend({
      ...
      ,error: function(e, code){
        ...
        if(e.responseJSON[response.statusName] == statusCode.logout){
          ...
          // var error = [
          //   '请求异常，请重试<br><cite>错误信息：'+ (e.responseJSON[response.msgName] ? e.responseJSON[response.msgName] : code) + '</cite>'
          //   // ,debug()
          // ].join('');
          // view.error(error);
          layer.msg(e.responseJSON[response.msgName] ? e.responseJSON[response.msgName] : code, {
            time: 3000
          });
        }
